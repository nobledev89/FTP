 ...
 * `transient`, `rate_limit`, and `invalid_output` back off and retry; `permanent_config` fails the
 * stage. Messages are shown in the console, so they name the command to run but never include a
 * path, token, or account email.
 */
